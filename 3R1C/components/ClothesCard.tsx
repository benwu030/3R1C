...
interface Props{
    item: { title: string, image: ImageSource, price: number, createdAt: Date },
    onPress?:()=>void
}
const ClothesCard = ({item:{image,title,price,createdAt},onPress}:Props) => {
  return (
    <TouchableOpacity onPress={onPress} className='flex-1  relative'>
      ...
      <View className='flex-col items-center justify-center mt-2'>
        ...
        <Text className='font-S-Medium text-beige-darker text-sm'>{createdAt.toLocaleString('en-GB')}</Text>
      </View>
    </View>
    </TouchableOpacity>
  )
