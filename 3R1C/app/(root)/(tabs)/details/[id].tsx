...
import { deleteClotheById, getClotheById, updateClotheImage } from "@/lib/CRUD/clotheCRUD";
...
const ClotheDetailsScreen = () => {
  ...
  const [absClotheImage,setAbsClotheImage] = useState(checkAbsoultePath(clothe?.localImageURL??"")) ;
  ...
  return (
    <SafeAreaView className="px-5 bg-sand-dark flex-1">
      ...
      <View className="flex-row justify-between items-center py-5 bg-beige rounded-t-lg">
        <TouchableOpacity onPress={() => {router.back() ;toggleEdit()}} className="basis-1/3">
          <Image source={icons.rightArrow} className="size-10 rotate-180" tintColor={"#b17457"}/>
        </TouchableOpacity>
    ...
    <TouchableOpacity onPress={editImage}>
     ...
     <Image
      ...
      <View className="flex-col items-start  mt-2  ">
        <Text className="font-S-RegularItalic text-lg ">Purchased Price</Text>
        <Text className="font-S-RegularItalic text-2xl ">${clothe!.price}</Text>
      </View>
      <View className="flex-col items-start mt-2 ">
      <Text className="font-S-RegularItalic text-lg">
        Purchased on
      </Text>
        <Text className="font-S-RegularItalic text-xl"> {clothe!.purchasedate
      ...
      <View className="flex-col items-start mt-2 ">
        ...
        <Text className="font-S-RegularItalic text-2xl">{clothe!.brand??"" }</Text>
      </View>
      <View className="flex-row justify-start items-center mt-2 gap-20">
      <View className="flex-col  items-start  ">
        ...
        <Text className="font-S-RegularItalic text-lg">{clothe!.maincategory}</Text>
      </View>
      <View className="flex-col items-start ">
        <Text className="font-S-RegularItalic text-lg">Sub Categories:</Text>
        <View className="flex-row flex-wrap">
          {clothe?.subcategories?.map((element: string) => (
            <Text key={element} className="font-S-RegularItalic text-lg mr-2">
              {element}
            </Text>
          ))}
        </View>
      ...
      <View className="flex-col  items-start  ">
        ...
        <Text className="font-S-RegularItalic text-lg border-2 border-oat rounded-lg w-full">{clothe!.remark??""}</Text>
      </View>
      </View>
      </ScrollView>
    </SafeAreaView>
